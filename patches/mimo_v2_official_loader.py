...
def _shard_fp8_qkv_proj(
    ...
    # The checkpoint's fp8 scales follow one of two layouts, detectable from
    ...
    if tp_size == 1:
        # Official MiMo-V2.6 checkpoint layout (reverse-engineered against
        # the ProCreations de-interleaved reference): FOUR export super-
        # groups, each holding what one TP=4 rank would own -
        #   [Q (num_heads/4 * head_dim rows) | K (kv/4 * head_dim) | V (kv/4 * v_head_dim)]
        # SWA (8 kv -> 2/group): 3712 rows/group = 29 whole blocks, scales
        # contiguous over the fused tensor (116 blocks). GA (4 kv -> 1/group):
        # 3392 rows/group = 26.5 blocks, scales padded per-group (27 each).
        G = 4
        hg = num_heads // G
        kg = num_kv_heads // G
        q_rows = hg * head_dim
        k_rows = kg * head_dim
        v_rows = kg * v_head_dim
        rows_per_group = q_rows + k_rows + v_rows
        total_rows = w_full.shape[0]

        # Per-absolute-row scales.
        contiguous = (s_full.shape[0] == (total_rows + block - 1) // block)
        s_f = s_full.to(torch.float32)
        if contiguous:
            row_scales = (
                s_f.repeat_interleave(block, dim=0)
                .repeat_interleave(block, dim=1)[:total_rows, : w_full.shape[1]]
            )
        else:
            gb = (rows_per_group + block - 1) // block  # 27 for GA
            row_scales = torch.empty(
                total_rows, w_full.shape[1], dtype=torch.float32, device=s_f.device
            )
            for g in range(G):
                base_w = g * rows_per_group
                base_s = g * gb
                n = min(rows_per_group, total_rows - base_w)
                seg = (
                    s_f[base_s : base_s + gb]
                    .repeat_interleave(block, dim=0)
                    .repeat_interleave(block, dim=1)
                )[:n]
                row_scales[base_w : base_w + n] = seg

        deq = w_full.to(torch.float32) * row_scales

        def sect(off, n):
            return torch.cat(
                [
                    deq[g * rows_per_group + off : g * rows_per_group + off + n]
                    for g in range(G)
                ],
                dim=0,
            )

        grouped = torch.cat([sect(0, q_rows), sect(q_rows, k_rows),
                             sect(q_rows + k_rows, v_rows)], dim=0)
        return scaled_quantize(
            grouped, GroupShape(block, block), w_full.dtype,
