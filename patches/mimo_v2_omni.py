...
def _transform_single(
    ...
    global _PROBE_DONE
    _PROBE_DONE = getattr(_transform_single, "_n", 0)
    _transform_single._n = _PROBE_DONE + 1
    if _PROBE_DONE < 3:
        try:
            import logging as _lg
            _lg.getLogger("vllm").warning(
                "PROBE img type=%s", type(img).__name__,
            )
            if isinstance(img, torch.Tensor):
                _lg.getLogger("vllm").warning(
                    "PROBE tensor dtype=%s shape=%s min=%.4f max=%.4f",
                    img.dtype, tuple(img.shape), float(img.min()), float(img.max()),
                )
            elif hasattr(img, "size"):
                _lg.getLogger("vllm").warning("PROBE PIL size=%s mode=%s", img.size, getattr(img, "mode", "?"))
                try:
                    import numpy as _np
                    _arr = _np.array(img.convert("RGB"))
                    _lg.getLogger("vllm").warning(
                        "PROBE channel means R=%.1f G=%.1f B=%.1f",
                        float(_arr[..., 0].mean()), float(_arr[..., 1].mean()), float(_arr[..., 2].mean()),
                    )
                except Exception:
                    pass
        except Exception:
    ...
    if isinstance(img, torch.Tensor):
        t = img.float()
        # vLLM hands the image branch float tensors in [0, 1]; the pixel
        # mean/std below are calibrated for [0, 255] (SGLang convention).
        # Without this rescale the chroma signal collapses to ~0.3 sigma and
        # the model reads colorful images as near-neutral (white/grey).
        if float(t.max()) <= 1.5:
            t = t * 255.0
        _, h, w = t.shape
